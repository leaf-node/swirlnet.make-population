...
makeGenePool = function (settings) {
    ...
    defaults.geneEnablementRate                 = 0.25;
    ...
    crossoverGenomes = function (genome1, genome2, fitness1, fitness2) {
        ...
        intersecting.forEach(function (innovation) {
            ...
            if (getGene(fitterGenome, innovation)[2] === false
                    || getGene(otherGenome, innovation)[2] === false) {
                newGene[2] = (Math.random() < settings.geneEnablementRate) ? true : false;
            }
        });
        ...
        if (fitness1 !== fitness2) {
            ...
            nonMatchingMatrix.forEach(function (nonMatchingInnovs, i) {
                nonMatchingInnovs.forEach(function (innovation) {

                    var genome, gene, upstreamInnovationNumber, downstreamInnovationNumber;

                    randomSelection = (Math.random() < settings.inheritRandomNonMatchingRate) ? true : false;

                    if (randomSelection) {

                        genome = (i === 0) ? fitterGenome : otherGenome;
                        gene = copyObj(getGene(genome, innovation));

                        setGene(newGenome, gene);

                        if (gene[1] === "connection") {
                            upstreamInnovationNumber = gene[3];
                            downstreamInnovationNumber = gene[4];

                            if (getGene(newGenome, upstreamInnovationNumber) === undefined) {
                                setGene(newGenome, copyObj(getGene(genome, upstreamInnovationNumber)));
                            }
                            if (getGene(newGenome, downstreamInnovationNumber) === undefined) {
                                setGene(newGenome, copyObj(getGene(genome, downstreamInnovationNumber)));
                            }
                        }
                    }
                });
