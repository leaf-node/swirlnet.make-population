...
makeNodeGene = function (subtype, activationFunction, position, upstream, downstream) {
    ...
    console.assert(((subtype === "bias" || subtype === "input" || subtype === "output")
            ...
            || (subtype === "hidden" && typeof upstream === "number" && !isNaN(upstream)
                && typeof downstream === "number" && !isNaN(downstream)),
            "swirljs: internal error: invalid node gene upstream node: " + upstream +
...
// used in making a node/neuron gene
makeNodeGeneHelper = function (innovationNumber, subtype, activationFunction, position) {
    ...
    console.assert(subtype === "bias" || subtype === "input" || subtype === "output" || subtype === "hidden",
            "swirljs: internal error: invalid node gene subtype: " + subtype);
    console.assert(typeof activationFunction === "string",
            "swirljs: internal error: invalid node gene activation function: " + activationFunction);
    console.assert(((subtype === "bias" || subtype === "hidden") && position === undefined)
            || ((subtype === "input" || subtype === "output") && typeof position === "number" && !isNaN(position)),
            "swirljs: internal error: invalid node gene position: " + position + " for node subtype: " + subtype);
...
makeConnectionGeneHelper = function (innovationNumber, upstream, downstream, weight) {
    ...
    var that, superThat, init, enabledState,
        ...
        enable, disable, setWeight, addWeight, copy, stringify;

    console.assert(typeof upstream === "number" && !isNaN(upstream),
            "swirljs: internal error: invalid connection gene upstream neuron: " + upstream);
    console.assert(typeof downstream === "number" && !isNaN(downstream),
            "swirljs: internal error: invalid connection gene downstream neuron: " + downstream);
    console.assert(typeof weight === "number" && !isNaN(weight),
            "swirljs: internal error: invalid connection gene weight: " + weight);
    ...
    addWeight = function (weightChange) {
        weight += weightChange;
    };
...
makeSuperGene = function (innovationNumber, type) {
    ...
    console.assert(typeof innovationNumber === "number" && !isNaN(innovationNumber),
            "swirljs: internal error: invalid gene innovation number: " + innovationNumber);
